...
Axiom H : (1%Z <= (((length s) - 1%Z)%Z + 1%Z)%Z)%Z.
...
Axiom H2 : (i <= ((length s) - 1%Z)%Z)%Z.

Axiom LoopInvariant :
  forall (j:Numbers.BinNums.Z), (0%Z <= j)%Z /\ (j < i)%Z ->
  is_lower_pt (mixfix_lbrb s r) (mixfix_lbrb s j).

Axiom LoopInvariant1 : is_index r s.

Parameter lpeqdtrp'result'unused'unused: Init.Datatypes.bool.

Axiom H3 : ~ ((y (mixfix_lbrb s i)) < (y rp))%R.

Axiom H4 :
  (((y (mixfix_lbrb s i)) = (y rp)) ->
   (lpeqdtrp'result'unused'unused = Init.Datatypes.true)) /\
  (~ ((y (mixfix_lbrb s i)) = (y rp)) ->
   (lpeqdtrp'result'unused'unused = Init.Datatypes.false)).

Axiom H5 :
  ~ (((y (mixfix_lbrb s i)) = (y rp)) /\
     (((x (mixfix_lbrb s i)) <= (x rp))%R \/
      ~ ((x (mixfix_lbrb s i)) <= (x rp))%R /\
...
Axiom H7 : (j < (i + 1%Z)%Z)%Z.

(* Why3 goal *)
Theorem lowest_leftmost'vc : is_lower_pt (mixfix_lbrb s r) (mixfix_lbrb s j).
Proof.
