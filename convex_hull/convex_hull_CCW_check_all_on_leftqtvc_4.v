...
Axiom all_on_left_ccw :
  forall (s:array pt) (k:Numbers.BinNums.Z) (i:Numbers.BinNums.Z)
    (j:Numbers.BinNums.Z),
  ~ (k = i) -> ~ (k = j) -> is_index k s -> is_index i s -> is_index j s ->
  all_on_left s i j ->
  ccw (mixfix_lbrb s i) (mixfix_lbrb s j) (mixfix_lbrb s k).

Axiom not_ccw_not_all_on_left :
  forall (s:array pt) (k:Numbers.BinNums.Z) (i:Numbers.BinNums.Z)
    (j:Numbers.BinNums.Z),
  ~ (k = i) -> ~ (k = j) -> is_index k s -> is_index i s -> is_index j s ->
  ~ ccw (mixfix_lbrb s i) (mixfix_lbrb s j) (mixfix_lbrb s k) ->
...
Axiom LoopInvariant2 : (0%Z < k)%Z -> is_index (k - 1%Z)%Z s.

Axiom LoopInvariant3 :
  (0%Z < k)%Z ->
  ccw (mixfix_lbrb s i) (mixfix_lbrb s j) (mixfix_lbrb s (k - 1%Z)%Z).

Parameter lpeqrp'result'unused'unused: Init.Datatypes.bool.

Axiom lpeqrp'result'unused'unused'def :
  ((k = i) -> (lpeqrp'result'unused'unused = Init.Datatypes.true)) /\
  (~ (k = i) -> (lpeqrp'result'unused'unused = Init.Datatypes.false)).

Parameter lpeqrp'result'unused'unused1: Init.Datatypes.bool.

Axiom H3 : ~ (k = i).

Axiom H4 :
  ((k = j) -> (lpeqrp'result'unused'unused1 = Init.Datatypes.true)) /\
  (~ (k = j) -> (lpeqrp'result'unused'unused1 = Init.Datatypes.false)).

Axiom H5 :
  ~ ((k = j) \/ ~ (k = j) /\ (Init.Datatypes.false = Init.Datatypes.true)).
...
Axiom H6 : ccw (mixfix_lbrb s i) (mixfix_lbrb s j) (mixfix_lbrb s k).

Axiom LoopInvariant4 :
  forall (l:Numbers.BinNums.Z), (0%Z <= l)%Z /\ (l < (k + 1%Z)%Z)%Z ->
  ~ (l = i) -> ~ (l = j) ->
  ccw (mixfix_lbrb s i) (mixfix_lbrb s j) (mixfix_lbrb s l).
