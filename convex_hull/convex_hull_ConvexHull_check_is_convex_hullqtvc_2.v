...
(* Why3 assumption *)
Definition distinct (p:path) : Prop :=
  forall (i:Numbers.BinNums.Z) (j:Numbers.BinNums.Z),
  is_valid_index i p /\ is_valid_index j p /\ (i < j)%Z ->
  ~ ((elements p i) = (elements p j)).
...
(* Why3 assumption *)
Definition wf_path (p:path) (s:array pt) : Prop :=
  distinct p /\
  (forall (i:Numbers.BinNums.Z), is_valid_index i p -> is_index (nth p i) s).

Parameter if_term: path -> Numbers.BinNums.Z -> Numbers.BinNums.Z.

Axiom if_term'def :
  forall (p:path) (i:Numbers.BinNums.Z),
  (((i + 1%Z)%Z = (size p)) -> ((if_term p i) = 0%Z)) /\
  (~ ((i + 1%Z)%Z = (size p)) -> ((if_term p i) = (i + 1%Z)%Z)).

(* Why3 assumption *)
Definition is_convex_hull (s:array pt) (p:path) : Prop :=
  wf_path p s -> (2%Z <= (length s))%Z -> forall (i:Numbers.BinNums.Z),
  is_valid_index i p -> all_on_left s (nth p i) (nth p (if_term p i)).

Parameter s: array pt.

Parameter p: path.

Axiom Requires : (2%Z <= (length s))%Z.

Axiom Requires1 : (2%Z <= (size p))%Z.

Axiom Requires2 : wf_path p s.

Axiom Ensures : ((nth p 1%Z) = (elements p 1%Z)).

Axiom Ensures1 : ((nth p 0%Z) = (elements p 0%Z)).

Axiom H : (0%Z <= (((size p) - 2%Z)%Z + 1%Z)%Z)%Z.

Parameter r: Init.Datatypes.bool.

Axiom LoopInvariant :
  forall (j:Numbers.BinNums.Z),
  (0%Z <= j)%Z /\ (j < (((size p) - 2%Z)%Z + 1%Z)%Z)%Z ->
  all_on_left s (nth p j) (nth p (j + 1%Z)%Z).

Axiom LoopInvariant1 :
  forall (j:Numbers.BinNums.Z),
  (0%Z <= j)%Z /\ (j < (((size p) - 2%Z)%Z + 1%Z)%Z)%Z ->
  is_valid_index j p /\ is_valid_index (j + 1%Z)%Z p.

Axiom LoopInvariant2 :
  (0%Z < (((size p) - 2%Z)%Z + 1%Z)%Z)%Z ->
  (r = Init.Datatypes.true) <->
  all_on_left s (nth p ((((size p) - 2%Z)%Z + 1%Z)%Z - 1%Z)%Z)
  (nth p (((size p) - 2%Z)%Z + 1%Z)%Z).

Axiom LoopInvariant3 : (r = Init.Datatypes.false) -> ~ is_convex_hull s p.

Axiom Assert : (r = Init.Datatypes.true).

(* Why3 goal *)
Theorem check_is_convex_hull'vc : is_convex_hull s p.
Proof.
